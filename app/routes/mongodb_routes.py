...
router = APIRouter(prefix="/MongoDB")

# ---------------------------
# Test conexión DB
# ---------------------------
@router.get("/check-db", tags=["Test Conexión DB"])
def check_db():
...
@router.get("/servicios/{servicio_id}", tags=["Servicios"])
...
@router.put("/servicios/{servicio_id}", tags=["Servicios"])
